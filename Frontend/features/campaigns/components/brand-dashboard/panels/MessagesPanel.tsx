...
export type BrandConversation = Conversation & { campaignTitle: string }
...
export function MessagesPanel({
  ...
  const threadsByCampaign = conversations.reduce<Record<string, BrandConversation[]>>((groups, conversation) => {
    groups[conversation.campaignTitle] = [...(groups[conversation.campaignTitle] ?? []), conversation]
    return groups
  }, {})
    ...
    <section className="grid min-h-[620px] gap-4 lg:grid-cols-[300px_1fr]">
      <aside className="rounded-[24px] border border-[#e8e2d9] bg-[#fbfaf7] p-3 shadow-sm">
        ...
        <div className="space-y-2">
          {Object.entries(threadsByCampaign).map(([campaignTitle, threads]) => (
            <div key={campaignTitle} className="rounded-[18px] bg-white/55 p-2">
              <p className="px-2 py-1 text-[11px] font-black uppercase tracking-[0.12em] text-[#8a8175]">{campaignTitle}</p>
              <div className="mt-1 space-y-1">
                {threads.map((conversation) => (
                  <button key={conversation.id} className={`flex w-full items-center gap-3 rounded-[16px] p-3 text-left transition ${activeConversation?.id === conversation.id ? "bg-white shadow-sm ring-1 ring-[#e8e2d9]" : "hover:bg-white/70"}`} type="button" onClick={() => onRoomChange(conversation.id)}>
                    <span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[#1f252b] text-xs font-black text-white">{conversation.creator.full_name.charAt(0)}</span>
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-black text-[#1f252b]">{conversation.creator.full_name}</span>
          ...
          {conversations.length === 0 && (
            <div className="rounded-[18px] border border-dashed border-[#ded8cf] bg-white p-4 text-center">
              ...
              <p className="mt-2 text-xs font-semibold text-[#69716b]">Messages appear after a creator is accepted.</p>
            </div>
          )}
        </div>
      </aside>
      ...
      <div className="flex min-h-0 flex-col overflow-hidden rounded-[24px] border border-[#e8e2d9] bg-[#fbfaf7] shadow-sm">
        <header className="border-b border-[#e8e2d9] p-5">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-xl font-black text-[#1f252b]">{activeConversation?.creator.full_name ?? "Messages"}</h2>
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-5">
          {roomMessages.map((item) => (
            <div key={item.id} className={item.sender_role === "brand" ? "ml-auto max-w-md" : "max-w-md"}>
              ...
              <p className={`rounded-[20px] px-4 py-3 text-sm font-semibold leading-6 ${item.sender_role === "brand" ? "bg-[#1f252b] text-white" : "bg-white text-[#505852] ring-1 ring-[#e8e2d9]"}`}>{item.body}</p>
            </div>
          ))}
          {roomMessages.length === 0 && (
            <div className="grid min-h-72 place-items-center rounded-[22px] border border-dashed border-[#ded8cf] bg-white/60 p-8 text-center">
              <div>
                ...
                <p className="mt-2 text-xs font-semibold text-[#69716b]">Send the first update to this creator.</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 border-t border-[#e8e2d9] bg-white/70 p-4">
          ...
          />
          <button className="grid size-11 place-items-center rounded-full bg-[#1f252b] text-white transition hover:bg-[#303840] disabled:opacity-50" type="button" aria-label="Send message" disabled={!activeConversation || !message.trim()} onClick={onSend}>
