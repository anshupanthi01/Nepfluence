...
export function MessagesPanel({
  ...
  const ownMessages = roomMessages.filter((item) => item.sender_role === "brand")
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-5">
          {roomMessages.map((item) => {
            ...
            <div key={item.id} className={isOwnMessage ? "ml-auto max-w-md" : "max-w-md"}>
              ...
              <p className={`rounded-[20px] px-4 py-3 text-sm font-semibold leading-6 ${isOwnMessage ? "bg-[#1f252b] text-white" : "bg-white text-[#505852] ring-1 ring-[#e8e2d9]"}`}>{item.body}</p>
            </div>
          )})}
